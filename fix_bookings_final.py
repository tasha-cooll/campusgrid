...
<div style="max-width:640px;">
  <div class="cg-card">
    ...
    <p style="font-size:0.82rem;color:var(--text-muted);margin-bottom:24px;">Select a facility and time slot. The system will check for conflicts automatically.</p>
    ...
    <div style="display:grid;gap:16px;">
      ...
      <div style="padding:16px;background:rgba(123,40,40,0.08);border:1px solid rgba(123,40,40,0.2);border-radius:6px;">
        ...
        <p style="font-size:0.78rem;color:var(--text-muted);margin-top:6px;margin-bottom:12px;">Priority bookings displace existing bookings. Use only for official university events.</p>
...
if (preSelected) {
  ...
  if (sel.value === preSelected) { onFacilityChange(); }
}
...
function onFacilityChange() {
  ...
  if (zones.length) {
    ...
    zOpts.innerHTML = zh;
    zSec.style.display = 'block';
  } else {
    zSec.style.display = 'none';
  }
  checkConflict();
}
...
async function submitBooking() {
  ...
  var payload = {
    ...
    purpose: pur,
    expected_attendance: parseInt(att),
  ...
  if (res) {
    ...
    if (res.ok) {
      ...
      btn.style.opacity = '1';
      btn.innerHTML = '<i class="bi bi-check-circle-fill me-1"></i> Submitted';
