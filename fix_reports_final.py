...
<div class="print-header" style="margin-bottom:20px;padding-bottom:12px;border-bottom:2px solid #C9A84C;">
  <div style="display:flex;align-items:center;gap:16px;">
    <div>
      ...
      <p style="margin:4px 0 0;font-size:0.85rem;color:#666;">University of Eastern Africa Baraton - School of Business</p>
      <p id="print-period" style="margin:4px 0 0;font-size:0.85rem;color:#666;"></p>
...
async function loadReports() {
  ...
  if (!utilRes) {
    container.innerHTML = '<div class="cg-alert cg-alert-error"><i class="bi bi-exclamation-circle me-2"></i>Unable to connect to the reports API. Please ensure you are logged in.</div>';
    return;
  }

  if (!utilRes.ok) {
    var errData = await utilRes.json().catch(function(){ return {}; });
    container.innerHTML = '<div class="cg-alert cg-alert-error"><i class="bi bi-exclamation-circle me-2"></i>Failed to load report data. Status: ' + utilRes.status + '. ' + (errData.detail || '') + '</div>';
    return;
  }
  ...
  if (!facs.length) {
    container.innerHTML = '<div class="cg-alert cg-alert-info"><i class="bi bi-info-circle me-2"></i>No booking data found for ' + MONTHS[parseInt(month)] + ' ' + year + '. Try selecting a different month.</div>';
    return;
  }
  ...
  html += '<div class="cg-card-title">Booking Status by Facility</div>';
  html += '<div style="position:relative;height:260px;margin-top:12px;">';
  html += '<canvas id="barChart"></canvas></div></div>';

  // ── DONUT + CONFLICT SIDE BY SIDE ───────────────────────────
  html += '<div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">';

  // Donut
  html += '<div class="cg-card">';
  html += '<div class="cg-card-title">Overall Booking Breakdown</div>';
  html += '<div style="position:relative;height:220px;margin-top:12px;">';
  html += '<canvas id="donutChart"></canvas></div>';
  ...
  if (conflictData && conflictData.total_conflicts > 0) {
    ...
    html += '<div style="font-size:0.72rem;color:var(--text-muted);text-transform:uppercase;letter-spacing:1px;margin-top:4px;">Total Conflicts Detected</div></div>';
    ...
    Object.entries(conflictData.by_facility).forEach(function(e) {
      ...
      html += '<div style="height:4px;width:' + pct + '%;background:' + GOLD + ';border-radius:2px;"></div></div></div>';
    });
    html += '</div></div>';
  } else {
    ...
    html += '<p style="margin-top:12px;font-size:0.85rem;">No conflicts detected this period.</p></div>';
  }
  html += '</div>';
  html += '</div>';
  ...
  // ── DATA TABLE ──────────────────────────────────────────────
  html += '<div class="cg-card">';
  html += '<div class="cg-card-title">Detailed Utilization Table</div>';
  html += '<div style="overflow-x:auto;margin-top:12px;">';
  html += '<table class="cg-table" id="util-table"><thead><tr>';
  ...
  facs.forEach(function(f) {
    ...
    html += '<div style="height:4px;width:' + (rate/2) + 'px;background:' + rateColor + ';border-radius:2px;"></div></div>';
...
function renderBarChart(facs) {
  ...
  if (!ctx) return;
  var labels = facs.map(function(f){ return f.facility; });
  barChart = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [
        {
          label: 'Approved',
          data:  facs.map(function(f){ return f.approved; }),
        ...
        {
          label: 'Pending',
          data:  facs.map(function(f){ return f.pending; }),
        ...
        {
          label: 'Rejected',
          data:  facs.map(function(f){ return f.rejected; }),
          backgroundColor: 'rgba(123,40,40,0.7)',
          borderColor:     'rgba(123,40,40,1)',
          borderWidth: 1,
          borderRadius: 3,
        },
        {
          label: 'Displaced',
          data:  facs.map(function(f){ return f.displaced; }),
    ...
    options: {
      ...
      plugins: {
        legend: {
          labels: { color: '#E8DCC8', font: { size: 11, family: 'Inter' }, boxWidth: 12 }
        },
        tooltip: {
      ...
      scales: {
        ...
        y: {
          beginAtZero: true,
          ticks: { color: '#8A9BBF', font: { size: 11 }, stepSize: 1 },
          grid:  { color: 'rgba(30,45,74,0.5)' }
        }
      }
    }
...
function exportCSV() {
  ...
  var month = document.getElementById('month-select').value;
  var year  = document.getElementById('year-select').value;
  var csv   = 'Facility,Total Bookings,Approved,Pending,Rejected,Displaced,Conflicts,Hours Booked,Approval Rate\n';
